...
export default function Dashboard() {
  ...
  const loadPersonaRuns = async (userId: string) => {
    ...
    const {
      data,
      error
    } = await supabase.from("persona_runs" as any).select("id, title, status, created_at, completed_at").eq("user_id", userId).order("created_at", {
      ascending: false
    });
    if (error) {
      toast({
  ...
  const handleDelete = async (id: string) => {
    const {
      error
    } = await supabase.from("persona_runs" as any).delete().eq("id", id);
    if (error) {
      toast({
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-6xl mx-auto">
          ...
          {loading ? <div className="text-center py-12">
            ...
            </div> : personaRuns.length === 0 ? <Card className="text-center py-12">
              <CardHeader>
              ...
              {personaRuns.map(run => <Card key={run.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
                  ...
                  <CardContent>
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" className="flex-1" onClick={() => navigate(`/persona-run/${run.id}`)}>
